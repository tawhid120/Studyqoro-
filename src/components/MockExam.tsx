...
export default function MockExam({ stats, setStats }: MockExamProps) {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {quizState === "setup" && (
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl max-w-3xl mx-auto shadow-xl">
          ...
          <div className="space-y-6">
                  ...
                  <button
                    ...
                    className={`px-4 py-3 rounded-xl text-xs font-bold text-left transition-all ${
                      selectedSubject === sub
                        ...
                        : "bg-slate-950 text-slate-400 hover:text-slate-100 hover:bg-slate-900 border border-slate-800/80"
                    }`}
                  >
                    {sub}
                  </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-300 block">প্রশ্ন সংখ্যা:</label>
                <div className="flex bg-slate-950 border border-slate-800 rounded-xl p-1 text-slate-100">
                  {[5, 10].map((num) => (
                    <button
                      key={num}
                      type="button"
                      ...
                      className={`flex-1 text-xs font-bold py-2.5 rounded-lg transition-all ${
                        totalQuestionsCount === num ? "bg-slate-800 text-white" : "text-slate-400"
                      }`}
                    >
                      {num} টি প্রশ্ন
                    </button>
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-300 block">সময়সীমা নির্ধারণ:</label>
                <div className="flex bg-slate-950 border border-slate-800 rounded-xl p-1 text-slate-100">
                  {[3, 5, 10].map((mins) => (
                    <button
                      key={mins}
                      ...
                      className={`flex-1 text-xs font-bold py-2.5 rounded-lg transition-all ${
                        timerMinutes === mins ? "bg-slate-800 text-white" : "text-slate-400"
                      }`}
                    >
                      {mins} মিনিট
                    </button>
            ...
            <div className="bg-slate-950 rounded-2xl border border-slate-800 p-4.5 flex gap-3 text-xs leading-relaxed text-slate-400">
              <Info className="w-5 h-5 text-emerald-400 shrink-0" />
              <div>
                <span className="font-bold text-slate-200 block">পরীক্ষার নিয়মাবলী ও পুরস্কার:</span>
                <p className="mt-1">
                  ১. পরীক্ষার সময় কাউন্টডাউন টাইমার স্বয়ংক্রিয়ভাবে চলমান থাকবে।
                  <br />
                  ২. ৮০% বা তার বেশি স্কোরে এক্সট্রা <span className="text-yellow-500 font-bold">+২৫ XP</span> মেগা বোনাস রয়েছে!
                  <br />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:order-2 space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-3xl text-center">
              <div className="flex justify-center mb-1 text-slate-400 text-xs font-medium uppercase tracking-wider">
                অবশিষ্ট সময় (Time Left)
              </div>
              <div className="flex items-center justify-center gap-2.5 text-3xl font-black font-mono text-red-400">
            ...
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl">
              <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-4">সব প্রশ্নের সূচক</h4>
              ...
              {/* Status information */}
              <div className="mt-6 pt-5 border-t border-slate-800/80 flex justify-between text-xs text-slate-400">
            ...
            <button
              ...
              className="w-full py-4 bg-red-600 hover:bg-red-500 text-white font-bold text-sm rounded-2xl flex items-center justify-center gap-2 shadow-lg"
            >
          ...
          <div className="lg:col-span-2 lg:order-1 space-y-6">
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl">
              ...
              <div className="flex justify-between items-center mb-5">
                <span className="text-xs bg-slate-950 text-slate-400 px-3.5 py-1.5 rounded-full border border-slate-800 font-bold">
              ...
              {/* Question Text */}
              <h3 className="text-lg sm:text-xl font-bold text-slate-100 leading-relaxed mb-6">
                    ...
                    <button
                      ...
                      className={`w-full p-4.5 rounded-2xl border text-left text-sm transition-all flex items-center gap-4 ${
                        isSelected
                          ...
                          : "bg-slate-950 border-slate-850 text-slate-400 hover:bg-slate-900 hover:border-slate-700/80"
                      }`}
                    >
                      <span className={`w-5.5 h-5.5 rounded-lg flex items-center justify-center text-xs font-bold ${
                        isSelected ? "bg-emerald-400 text-slate-950" : "bg-slate-850 text-slate-500"
                      }`}>
              ...
              <div className="flex justify-between items-center mt-8 pt-6 border-t border-slate-850">
                <button
                  ...
                  className="px-4 py-2 bg-slate-950 text-slate-400 hover:text-slate-200 border border-slate-800 rounded-xl text-xs font-bold flex items-center gap-1.5 disabled:opacity-40 disabled:cursor-not-allowed"
                >
                ...
                {currentQuestionIndex < currentExam.totalQuestions - 1 ? (
                  <button
                    onClick={() => setCurrentQuestionIndex(prev => prev + 1)}
                    className="px-4 py-2 bg-slate-950 text-slate-200 hover:text-white border border-slate-800 rounded-xl text-xs font-bold flex items-center gap-1.5"
                  >
                  ...
                  <button
                    onClick={handleCompleteExam}
                    className="px-5 py-2 bg-emerald-500 text-slate-950 font-bold rounded-xl text-xs flex items-center gap-1.5"
                  >
      ...
      {quizState === "results" && currentExam && (
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl max-w-4xl mx-auto shadow-xl">
          ...
          <div className="text-center mb-8">
            ...
            <h3 className="text-2xl font-black text-slate-100 mt-4">মডেল টেস্ট সম্পন্ন হয়েছে!</h3>
            <p className="text-xs text-slate-400 mt-1">অভিনন্দন! তোমার দক্ষতার মূল্যায়ণ নিচে দেওয়া হলো।</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 text-center">
            ...
            <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800">
              <span className="text-xs text-slate-400 font-medium block">সঠিক উত্তর</span>
            ...
            <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800">
              <span className="text-xs text-slate-400 font-medium block">গড় নির্ভুলতা (Accuracy)</span>
              <span className="text-3xl font-black text-cyan-400 block mt-2">
                {currentExam.score}%
              </span>
            </div>

            <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800">
              <span className="text-xs text-slate-400 font-medium block">অর্জিত পড়াশোনা এক্সপি</span>
              <span className="text-3xl font-black text-yellow-400 block mt-2 flex items-center justify-center gap-1">
          ...
          <div className="space-y-4 mb-8">
            <h4 className="text-sm font-bold text-slate-200">প্রশ্নের সমাধান সমূহ (Detailed Solutions):</h4>
            ...
            {currentExam.questions.map((q, qI) => {
              ...
              return (
                <div key={q.id} className={`p-5 rounded-2xl border text-slate-300 text-sm leading-relaxed bg-slate-950 ${
                  isCorrect ? "border-emerald-500/20" : "border-red-500/20"
                }`}>
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-xs font-bold text-slate-300">প্রশ্ন {qI + 1}</span>
                  ...
                  <p className="font-bold text-slate-100 mb-3">{q.questionText}</p>
                  
                  <div className="text-xs space-y-1 bg-slate-900/60 p-3.5 rounded-xl border border-slate-850">
                    <p className="text-slate-400">১. তোমার উত্তর: <span className={isCorrect ? "text-emerald-400 font-semibold" : "text-red-400 font-medium"}>
                      {uSelection !== undefined ? q.options[uSelection] : "উত্তর দেওয়া হয়নি"}
                    </span></p>
                    <p className="text-slate-400">২. সঠিক উত্তর: <span className="text-emerald-400 font-bold">{q.options[q.correctIndex]}</span></p>
                    
                    <div className="mt-3.5 pt-3 border-t border-slate-800">
                      <span className="text-emerald-400 font-bold block mb-1">সমাধান উক্তি:</span>
                      <p className="text-xs text-slate-300 leading-normal">{q.explanation}</p>
                    </div>
                  </div>
                </div>
              );
          ...
          <button
            ...
            className="w-full py-4 bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 active:scale-95 transition-all"
          >
